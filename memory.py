...
class DKVMNHeadGroup(nn.Module):
    ...
    def __init__(self, memory_size, memory_state_dim, is_write):
        ...
        if self.is_write:
            ...
            self.add = torch.nn.Linear(self.memory_state_dim*2, self.memory_state_dim, bias=True)
            ...
            self.liner = torch.nn.Linear(self.memory_state_dim, self.memory_state_dim, bias=True)
    @staticmethod
    ...
    def write(self, control_input, memory, write_weight=None):
        ...
        if write_weight is None:
            write_weight = self.addressing(control_input=control_input, memory=memory) # control_input shape [64,200]     memory shape [64,20,200]
        ######### 修改部分 ###########
        C_t = control_input.view(control_input.size(0),1,control_input.size(1)).repeat(1, 20, 1) # C_t shape [64,20,200]
        control_plus_memory = C_t + memory # memory [64,20,200]
        Z_t = torch.sigmoid(self.liner(control_plus_memory))  # z_t [64,20,200]
        D_t = Z_t * memory  # shape [64,20,200]
        D_t_connect_C_t = torch.cat([D_t,C_t], 2)

        tile_V_t  = torch.tanh(self.add(D_t_connect_C_t))
        U_t = torch.sigmoid(self.erase(control_plus_memory))
        new_memory = (1 - U_t) * memory + U_t * tile_V_t
